...
export const DailyView: React.FC<DailyViewProps> = ({ currentDate, aulas, onEdit }) => {
  const { getSubjectProgress, getCourseProgress } = useSchedule();
          ...
          {processedClasses.map((aula) => {
            ...
            return (
              ...
              >
                ...
                {/* Workload Progress */}
                {!isVeryShort && (
                  <div className="mt-2 pt-1.5 border-t border-gray-100 space-y-1 print:border-gray-300">
                    {(() => {
                      const subjectProg = getSubjectProgress(aula.materia, aula.curso);
                      const courseProg = getCourseProgress(aula.curso);
